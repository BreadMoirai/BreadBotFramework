...
public interface CommandResultHandler<T> {
    ...
    static <T> void handleObject(CommandResultHandler<T> handler, CommandHandle command, CommandEvent event, Object result) {
        //noinspection unchecked
        T cast = (T) result;
        handler.handleResult(command, event, cast);
    }

    void handleResult(CommandHandle command, CommandEvent event, T result);
